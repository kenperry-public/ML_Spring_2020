...
class VanderPlas():
    ...
    def digits_plot(self, data, save_file=None):
        """
    ...
    def digits_reconstruction(self, data, n_components=None, save_file=None):
        """
        ...
        _ = self.digits_plot(reconstructed, save_file)

        return pca, projected, reconstructed

    def digits_show_clustering(self, projected, targets, alpha=0.9, save_file=None):
    ...
    def digits_subset(self, digits, subset, n_components=2, save_file=None):
        """
        Parameters
        ----------
        ...
        digits_pca_t, digits_projected_t, digits_reconstructed_t = self.digits_reconstruction(digits_t, n_components=n_components, save_file=save_file)
    ...
    def digits_subset_show_clustering(self, projected_t, targets_t, save_file=None):
        m = (targets_t <= 9)
        #m = np.isin( targets_t, [1, 4, 7])
        
        d, t = projected_t[ m ], targets_t[m]
        self.digits_show_clustering(d, t, save_file=save_file)
